...
# A_BX_EmbeddedSW_SRREMCTRL_0006        Check that module is still in controlled after recovering from losing BT connection
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRREMCTRL_0006
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRREMCTRL_0006"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check that module is still in controlled after recovering from losing BT connection" % test_ID
    print "***********************************************************************************************************************"
    ...
    print '\nOn AUX1...'
    print 'Step 5: Execute AT command on remote BC Smart server'
    ...
    print '\nOn DUT...'
    print 'Step 6: Disconnect BLE connection'
    ...
    print '\nOn AUX1...'
    print 'Step 7: Try to send remote command to DUT after disconnect BLE connection'
    SagSendAT(aux1_com, 'AT+SRBCSMARTCMD=1,"%s\\0d"\r' % command)
    SagWaitnMatchResp(aux1_com, ['\r\n+CME ERROR: 922\r\n'], 2000)
    ...
    print '\nOn DUT...'
    print 'Step 9: Active BLE connection to AUX1'
    SagSendAT(uart_com, 'AT+SRBLECNX=1\r')
    if not SagWaitnMatchResp(uart_com, ['\r\n+SRBLE_IND: 1,1\r\nOK\r\n'], 5000):
        raise Exception("---->Problem: DUT cannot connect to AUX1 properly !!!")
    SagWaitnMatchResp(uart_com, ['+SRBLEMTU: 1,%s\r\n' % mtu], 2000)
    ...
    print '\nOn AUX1...'
    print 'Step 10: Execute AT command on remote BC Smart server'
    SagSendRemoteAT(aux1_com, 1, command, mtu)
    SagWaitnMatchResp(uart_com, ['+SRREMCMD: "%s"\r\n' % command.replace('"', '\\22')], 2000)
    SagWaitnMatchResp(uart_com, expect_response, 2000)
    SagWaitnMatchRemoteResp(aux1_com, 1, expect_response, 5000)
    ...
    print '\nStep 13: Delete BLE configuration'
    print 'On DUT...'
