...
# A_BX_EmbeddedSW_SRSPP_DATA_0001               Check if module can receive data from remote device while being in data mode
# 
# Requirement
# 2 Euler modules
#    
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRSPP_DATA_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRSPP_DATA_0001"
...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s: Check if module can receive data from remote device while being in data mode" % test_ID
    print "*****************************************************************************************************************"
    ...
    print "\nStep 5: DUT: Active SPP session\n"
    SagSendAT(uart_com, 'AT+SRSPPCNX=1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTPAIR: "%s",1\r\n' %aux1_bluetooth_address], 5000)
    SagWaitnMatchResp(uart_com, ['\r\n+SRSPPCNX: 1,1,*\r\n'], 5000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 5000)
    
    print "\nStep 6: AUX1: Check URC\n"
    SagWaitnMatchResp(aux1_com, ['\r\n+SRBTPAIR: "%s",1\r\n' %dut_bluetooth_address], 5000)
    SagWaitnMatchResp(aux1_com, ['\r\n+SRBTCFG: 1,0,"%s",SPP,0\r\n' %dut_bluetooth_address], 5000)
    SagWaitnMatchResp(aux1_com, ['+SRSPPCNX: 1,1,*\r\n'], 5000)

    print "\nStep 7: DUT: Change DUT to connectable and discoverable mode\n"
    SagSendAT(uart_com, 'AT+SRBTSTATE=1,2\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 8: DUT: Put DUT to data mode to send data to AUX1\n"
    SagSendAT(uart_com, 'AT+SRSPPSTART=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nCONNECT\r\n'], 5000)
    SagSendAT(uart_com, 'Testing data mode')
    SagWaitnMatchResp(aux1_com, ['+SRSPP_DATA: 1,17,Testing data mode\r\n'], 5000)
    
    print "\nStep 9: On AUX1: Send data to DUT\n"
    SagSendAT(aux1_com, 'AT+SRSPPSND=1, "Hello DUT"\r')
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 5000)
    
    SagWaitnMatchResp(uart_com, ['Hello DUT'], 5000)
    
    print "\nStep 10: On DUT: Send data back to AUX1 then escapse data mode\n"
    SagSendAT(uart_com, 'Hello AUX1')
    SagWaitnMatchResp(aux1_com, ['+SRSPP_DATA: 1,10,Hello AUX1\r\n'], 5000)
    SagSendAT(uart_com, '+++')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 11: AUX1: Close SPP session to DUT\n"
    ...
    print "\nStep 13: Delete SPP session to DUT\n"
