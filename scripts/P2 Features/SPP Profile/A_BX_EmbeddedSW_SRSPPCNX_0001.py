...
# A_BX_EmbeddedSW_SRSPPCNX_0001                 Check if module can be received a SPP connection after reaching max support session
# 
# Requirement
# 2 Euler modules
#    
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRSPPCNX_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRSPPCNX_0001"
...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s: Check if module can be received a SPP connection after reaching max support session" % test_ID
    print "*****************************************************************************************************************"
    
    print "\nStep 1: DUT: Query SPP configure\n"
    ...
    print "\nStep 4: DUT: Change DUT to connectable and discoverable mode\n"
    SagSendAT(uart_com, 'AT+SRBTSTATE=1,2\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 5: AUX1: configure a SPP session with DUT BT address\n"
    SagSendAT(aux1_com, 'AT+SRSPPCFG=%s\r' %dut_bluetooth_address)
    SagWaitnMatchResp(aux1_com, ['\r\n+SRBTCFG: 1,0,"%s",SPP,0\r\n' %dut_bluetooth_address], 2000)
    SagWaitnMatchResp(aux1_com, ['OK\r\n'], 2000)
    
    print "\nStep 6: AUX: Change AUX to connectable and discoverable mode\n"
    ...
    print "\nStep 9: AUX1: Try to close the SPP session\n"
    SagSendAT(uart_com, 'AT+SRSPPCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 922\r\n'], 2000)
    
    print "\nOn AUX1: Check URC: An indicator appears to notify SPP connection has been closed\n"
    SagWaitnMatchResp(aux1_com, ['\r\n+SRSPPCLOSE: 1,0\r\n'], 5000)
