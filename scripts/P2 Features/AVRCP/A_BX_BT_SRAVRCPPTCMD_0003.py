...
# A_BX_BT_SRAVRCPPTCMD_0003                     Check all supported <cmd> and <key_state> return
# Requirement
# 1 Euler module
#    
# Author: ptnlam
...
# -----------------------------------------------------------------------------------
# A_BX_BT_SRAVRCPPTCMD_0003
# -----------------------------------------------------------------------------------

test_ID = "A_BX_BT_SRAVRCPPTCMD_0003"
...
try:
    ...
    print "\nStep 6: Configure a AVRCP session\n"
    SagSendAT(uart_com, 'AT+SRAVRCPCFG=%s\r' %bt_addr)
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTCFG: 1,0,"%s",AVRCP\r\n' %bt_addr], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 7: Configure a A2DP session\n"
    SagSendAT(uart_com, 'AT+SRA2DPCFG=%s\r' %bt_addr)
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTCFG: 2,0,"%s",A2DP\r\n' %bt_addr], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 8: Active A2DP session (AVRCP also active)\n"
    ...
    print "\nStep 9: Connect to device, query AVRCP session\n"
    SagSendAT(uart_com, 'AT+SRAVRCPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTCFG: 1,1,"%s",AVRCP\r\n' %bt_addr], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
    
    print "\nStep 10: Send AVRCP Passthrough command with cmd=0,key_state=0,1,2\n"
    for i in (0,1,2):
        SagSendAT(uart_com, 'AT+SRAVRCPPTCMD=1,0,%s\r' %i)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
        if i==0:
            SagWaitnMatchResp(uart_com, ['\r\n+SRAVRCPTCMD: 0,%s\r\n' %i], 5000)
            SagWaitnMatchResp(uart_com, ['\r\n+SRA2DPAUDIOSTATE: 2\r\n'], 5000)
        if i==2:
            SagWaitnMatchResp(uart_com, ['+SRAVRCPTCMD: 0,0\r\n'], 5000)
            SagWaitnMatchResp(uart_com, ['+SRAVRCPTCMD: 0,1\r\n'], 5000)
            SagWaitnMatchResp(uart_com, ['\r\n+SRA2DPAUDIOSTATE: 0\r\n'], 5000)
        else:
            SagWaitnMatchResp(uart_com, ['\r\n+SRAVRCPTCMD: 0,%s\r\n' %i], 5000)

    print "\nStep 11: Repeat Step 7,8 with cmd = 1 2 3 4 5 6\n"
    for cmd in (1,2,3,4,5,6):
        for key_state in (0,1,2):
            SagSendAT(uart_com, 'AT+SRAVRCPPTCMD=1,%s,%s\r' %(cmd,key_state))
            if key_state!= 2:
                SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
                SagWaitnMatchResp(uart_com, ['\r\n+SRAVRCPTCMD: %s,%s\r\n' %(cmd,key_state)], 5000)
            else:
