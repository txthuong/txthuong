...
# A_BX_BT_SRAVRCPREQMTD_0002                    Check +SRAVRCPREQMTD incase the session is not configured and not connected
# Requirement
# 1 Euler module
#    
# Author: ptnlam
...
# -----------------------------------------------------------------------------------
# A_BX_BT_SRAVRCPREQMTD_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_BT_SRAVRCPREQMTD_0002"
...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s: Check +SRAVRCPREQMTD incase the session is not configured and not connected" % test_ID
    print "*****************************************************************************************************************"
    ...
    print "\nStep 6: Check +SRAVRCPREQMTD write command with not configured session\n"
    for metadata in (0,1,2,3,4,5,6):
        SagSendAT(uart_com, 'AT+SRAVRCPREQMTD=1,%s\r'%metadata)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 910\r\n'], 2000)

    print "\nStep 7: Configure a AVRCP session with bt_addr\n"
    ...
    print "\nStep 9: Query AVRCP session (No configuration exist)\n"
    SagSendAT(uart_com, 'AT+SRAVRCPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTCFG: 1,0,"%s",AVRCP\r\n' %bt_addr], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)

    print "\nStep 10: Check +SRAVRCPREQMTD write command with not configured session\n"
    for metadata in (0,1,2,3,4,5,6):
        SagSendAT(uart_com, 'AT+SRAVRCPREQMTD=1,%s\r'%metadata)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 922\r\n'], 2000)
    
    print "\nStep 11: Active A2DP session (AVRCP also active)\n"
    ...
    print "\nStep 12: Connect to device, query AVRCP session\n"
    SagSendAT(uart_com, 'AT+SRAVRCPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+SRBTCFG: 1,1,"%s",AVRCP\r\n' %bt_addr], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
    
    print "\nStep 13: Check +SRAVRCPREQMTD write command with valid parameters\n"
    for metadata in (0,1,2,3,4,5,6):
        SagSendAT(uart_com, 'AT+SRAVRCPREQMTD=1,%s\r'%metadata)
        if metadata != 5:
            SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
