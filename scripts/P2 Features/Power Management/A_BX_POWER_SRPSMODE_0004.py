...
# A_BX_POWER_SRPSMODE_0004                 Check if setting backs to default after module wake up from deep sleep mode
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_POWER_SRPSMODE_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_POWER_SRPSMODE_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print "%s: Check if setting backs to default after module wake up from deep sleep mode" % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 14: Perform +SRPSMODE to put module to deep sleep mode"
    wait_time = timer * 1000 + 2000
    SagSendAT(uart_com, 'AT+SRPSMODE=1\r')
    print "\nWaiting for module wake up from deep sleep mode after %s seconds" % timer
    SagWaitnMatchResp(uart_com, ['*\r\nREADY\r\n'], wait_time)
    ...
    if not SagWaitnMatchResp(uart_com, ['\r\n+SRPSCFG: 0,0,0'], 2000):
        print '----> Problem: The configure did not back to default after waking up from deep sleep mode !!!'
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    SagSendAT(uart_com, "AT+KHTTPCFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
        print '----> Problem: The HTTP configure did not back to default after waking up from deep sleep mode !!!'
    SagSendAT(uart_com, "AT+KHTTPSCFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
        print '----> Problem: The HTTPS configure did not back to default after waking up from deep sleep mode !!!'
    SagSendAT(uart_com, "AT+KTCPCFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
        print '----> Problem: The TCP configure did not back to default after waking up from deep sleep mode !!!'
    SagSendAT(uart_com, "AT+SRSPPCFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
        print '----> Problem: The SPP configure did not back to default after waking up from deep sleep mode !!!'
    SagSendAT(uart_com, "AT+SRBLECFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
        print '----> Problem: The BLE configure did not back to default after waking up from deep sleep mode !!!'
