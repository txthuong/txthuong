...
# A_BX_BLE_SRBLENOTIFY_0002                     Check +SRBLENOTIFY with not existing and not connected session
# 
# Requirement
# 1 Euler module
#    
...
# -----------------------------------------------------------------------------------
# A_BX_BLE_SRBLENOTIFY_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_BLE_SRBLENOTIFY_0002"
...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s: Check +SRBLENOTIFY with not existing and not connected session" % test_ID
    print "*****************************************************************************************************************"
    ...
    print "\nStep 4: Perform +SRBLENOTIFY with not configured session\n"
    SagSendAT(uart_com, 'AT+SRBLENOTIFY=1,52,"\\00\\01\\02\\03"\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 5: DUT: Create a BLE session to AUX module\n"
    SagSendAT(uart_com, 'AT+SRBLECFG=%s\r' %aux1_bluetooth_addr)
    SagWaitnMatchResp(uart_com, ['\r\n+SRBLECFG: 1,0,"%s",23\r\n' %aux1_bluetooth_addr], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
    
    print "\nStep 6: Perform +SRBLENOTIFY without active BLE session\n"
    SagSendAT(uart_com, 'AT+SRBLENOTIFY=1,52,"\\00\\01\\02\\03"\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 7: DUT: Initiate connection to device B\n"
    ...
    print "\nStep 12: AUX1: Stop advertising\n"
    SagSendAT(aux1_com, 'AT+SRBLEADV=0\r')
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 13: Delete the BLE configure\n"
