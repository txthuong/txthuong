...
# A_BX_EmbeddedSW_KMQTTCNX_0001      To check syntax and basic functionality for the AT command "+KMQTTCNX", start MQTT Connection
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTCNX_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTCNX_0001"
...
try:
    ...
    SagSendAT(uart_com, "AT+KMQTTCNX=?\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 5: Check +KMQTTCNX execute command"
    SagSendAT(uart_com, "AT+KMQTTCNX\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 6: Check +KMQTTCNX read command"
    SagSendAT(uart_com, "AT+KMQTTCNX?\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 7: Check +KMQTTCNX write command with not configured session"
    session_id = [0, 2, 65535]
    for session in session_id:
        SagSendAT(uart_com, "AT+KMQTTCNX=%s\r" % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 910\r\n'], 2000)

    print "\nStep 8: Check +KMQTTCNX write command with out-range and invalid parameter <session_id>"
    session_id = [-1, 65536, 'a', 'A', '$', '*']
    for session in session_id:
        SagSendAT(uart_com, "AT+KMQTTCNX=%s\r" % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)

    print "\nStep 9: Check +KMQTTCNX write command with missing parameter"
    SagSendAT(uart_com, 'AT+KMQTTCNX=\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print "\nStep 10: Check +KMQTTCNX write command with extra parameter"
    SagSendAT(uart_com, 'AT+KMQTTCNX=1,1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 915\r\n'], 2000)
