...
# A_BX_EmbeddedSW_KMQTTDEL_0001                 Check Syntax read, execute +KMQTTDEL command with valid values, invalid values and values out of range
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTDEL_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTDEL_0001"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check Syntax read, execute +KMQTTDEL command with valid values, invalid values and values out of range' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 4: Check +KMQTTDEL test command"
    SagSendAT(uart_com, "AT+KMQTTDEL=?\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 5: Check +KMQTTDEL execute command"
    SagSendAT(uart_com, "AT+KMQTTDEL\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 6: Check +KMQTTDEL read command"
    SagSendAT(uart_com, "AT+KMQTTDEL?\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 7: Check +KMQTTDEL write command with missing parameter"
    SagSendAT(uart_com, 'AT+KMQTTDEL=\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print "\nStep 8: Check +KMQTTDEL write command with extra parameter"
    SagSendAT(uart_com, 'AT+KMQTTDEL=1,1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 915\r\n'], 2000)

    print "\nStep 9: Check +KMQTTDEL write command with not configured session"
    session_id = [0, 2, 65535]
    for session in session_id:
        SagSendAT(uart_com, "AT+KMQTTDEL=%s\r" % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 910\r\n'], 2000)

    print "\nStep 10: Check +KMQTTDEL write command with out-range and invalid parameter <session_id>"
    session_id = [-1, 65536, 'a', 'A', '$', '*']
    for session in session_id:
        SagSendAT(uart_com, "AT+KMQTTDEL=%s\r" % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)

    print "\nStep 11: Check +KMQTTDEL write command with valid session_id"
