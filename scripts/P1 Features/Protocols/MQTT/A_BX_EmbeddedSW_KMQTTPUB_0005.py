...
# A_BX_EmbeddedSW_KMQTTPUB_0005      Check that module support payload size maximum 80 bytes
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTPUB_0005
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTPUB_0005"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that module support payload size maximum 80 bytes ' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 5: Subscribe the topic on server"
    SagSendAT(uart_com, 'AT+KMQTTSUB=1,"%s",1\r' % topic)
    resp = SagWaitResp(uart_com, [''], 4000)
    SagMatchResp(resp, ['\r\nOK\r\n\r\n+KMQTTSUB: "%s","*"\r\n' % topic, '\r\nOK\r\n'])

    print "\nStep 6: Publish messages to the topic"
    while len(message) <= max:
        print "Payload size: %s" % len(message)
        SagSendAT(uart_com, 'AT+KMQTTPUB=1,"%s",2,0,"%s"\r' % (topic, message))
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)
        SagWaitnMatchResp(uart_com, ['\r\n+KMQTTSUB: "%s","%s"\r\n' % (topic, message)], 2000)
        message = message + random.choice(string.letters)

    print "\nStep 7: Publish a %s bytes message to the topic" % (max+1)
    SagSendAT(uart_com, 'AT+KMQTTPUB=1,"%s",2,0,"%s"\r' % (topic, message))
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 5000)

    print "\nStep 8: Close MQTT session"
    SagSendAT(uart_com, 'AT+KMQTTCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 9: Delete MQTT connection"
