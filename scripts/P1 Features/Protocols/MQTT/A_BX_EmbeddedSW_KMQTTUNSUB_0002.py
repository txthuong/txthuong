...
# A_BX_EmbeddedSW_KMQTTUNSUB_0002   Check that +KMQTTUNSUB can unsubscribe a specific  topic identified by the <topic_name> 
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTUNSUB_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTUNSUB_0002"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KMQTTSUB can subscribe a topic from specific MQTT session' % test_ID
    print "***************************************************************************************************************"
    ...
    for version in (3, 4):
        ...
        print "\nStep 7: Un-subscribe the topic"
        SagSendAT(uart_com, 'AT+KMQTTUNSUB=1,"%s"\r' % topic)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

        print "\nStep 8: Publish message to the subscribed topic"
        SagSendAT(uart_com, 'AT+KMQTTPUB=1,"%s",1,0,"%s"\r' % (topic, message))
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

        print "\nStep 9: Close the MQTT connection"
