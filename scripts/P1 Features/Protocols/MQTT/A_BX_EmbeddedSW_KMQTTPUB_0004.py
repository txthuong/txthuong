...
# A_BX_EmbeddedSW_KMQTTPUB_0004      Check if module can publish a zero byte retained message
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTPUB_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTPUB_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check if module can publish a zero byte retained message' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 5: Publish a retained message to the topic"
    SagSendAT(uart_com, 'AT+KMQTTPUB=1,"%s",1,1,"%s"\r' % (topic, message))
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 6: Subscribe the topic on server"
    SagSendAT(uart_com, 'AT+KMQTTSUB=1,"%s",1\r' % topic)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)
    SagWaitnMatchResp(uart_com, ['\r\n+KMQTTSUB: "%s","%s"\r\n' % (topic, message)], 2000)

    print "\nStep 7: UnSubscribe the topic"
    SagSendAT(uart_com, 'AT+KMQTTUNSUB=1,"%s"\r' % topic)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 8: Publish a zero byte retained message to the topic"
    SagSendAT(uart_com, 'AT+KMQTTPUB=1,"%s",1,1,""\r' % topic)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 9: Subscribe the topic"
    SagSendAT(uart_com, 'AT+KMQTTSUB=1,"%s",1\r' % topic)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)
    resp = SagWaitResp(uart_com, ['\r\n+KMQTTSUB: "%s","*"\r\n' % topic], 2000)
    if '\r\n+KMQTTSUB: "%s"' % topic in resp:
        VarGlobal.statOfItem = "NOK"
        print '----> Problem: Retained message still exist !!!'

    print "\nStep 10: Close MQTT session"
    SagSendAT(uart_com, 'AT+KMQTTCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 11: Delete MQTT connection"
