...
# A_BX_EmbeddedSW_KMQTTCNX_0004      Check if  +KMQTTCNX command can start a MQTT by using username, password for authentication
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTCNX_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTCNX_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check if  +KMQTTCNX command can start a MQTT by using username, password for authentication' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 2: Configure a MQTT connection with a basic authentication username, password"
    ...
    print "\nStep 5: Configure a MQTT connection without authentication"
    SagSendAT(uart_com, 'AT+KMQTTCFG=0,"%s",%s,4,"BX_DUT"\r' % (mqtt_server, mqtt_port))
    SagWaitnMatchResp(uart_com, ['\r\n+KMQTTCFG: 2\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 6: Query MQTT connection configuration"
    SagSendAT(uart_com, 'AT+KMQTTCFG?\r')
    SagWaitnMatchResp(uart_com, ['*\r\n+KMQTTCFG: 2,"%s",%s,4,"BX_DUT",60,1,0,"","",0,0,"",""\r\n' % (mqtt_server, mqtt_port)], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 7: Start MQTT connection"
    SagSendAT(uart_com, 'AT+KMQTTCNX=2\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 8: Close the MQTT connection"
    SagSendAT(uart_com, 'AT+KMQTTCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    SagSendAT(uart_com, 'AT+KMQTTCLOSE=2\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 9: Delete the MQTT connection"
    SagSendAT(uart_com, 'AT+KMQTTDEL=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    SagSendAT(uart_com, 'AT+KMQTTDEL=2\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 10: Query MQTT connection configuration"
    SagSendAT(uart_com, 'AT+KMQTTCFG?\r')
