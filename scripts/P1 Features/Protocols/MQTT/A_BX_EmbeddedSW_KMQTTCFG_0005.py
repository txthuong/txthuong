...
# A_BX_EmbeddedSW_KMQTTCFG_0005      Check that module support MQTT connection with TLS
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTCFG_0005
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTCFG_0005"
...
try:
    ...
    clientid = dut_bt_name
    dut_lwt_topic = 'TMA/DUT/status'
    dut_lwt_message = '%s OFFLINE' % clientid

    print "\nStep 2: Configure a MQTT connections with TLS"
    SagSendAT(uart_com, 'AT+KMQTTCFG=1,"%s",%s,4,"%s",60,1,1,"%s","%s",0,0,"%s","%s"\r' % (mqtt_server, mqtt_tls_port, clientid, dut_lwt_topic, dut_lwt_message, mqtt_user, mqtt_password))
    SagWaitnMatchResp(uart_com, ['\r\n+KMQTTCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query MQTT connection configuration"
    SagSendAT(uart_com, 'AT+KMQTTCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KMQTTCFG: 1,"%s",%s,4,"%s",60,1,1,"%s","%s",0,0,"%s","%s"\r\n' % (mqtt_server, mqtt_tls_port, clientid, dut_lwt_topic, dut_lwt_message, mqtt_user, mqtt_password)], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
