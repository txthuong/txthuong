...
# A_BX_EmbeddedSW_KMQTTCLOSE_0002               Check that +KMQTTCLOSE command can disconnect  from MQTT broker properly
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KMQTTCLOSE_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KMQTTCLOSE_0002"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KMQTTCLOSE command can disconnect  from MQTT broker properly' % test_ID
    print "***************************************************************************************************************"
    ...
    message = 'Testing MQTT on BX301x'
    ...
    for session in range (1, max_session):
        ...
        print "\nStep 5: Publish message to the topic"
        SagSendAT(uart_com, 'AT+KMQTTPUB=%s,"%s",1,0,"%s"\r' % (session, topic, message))
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

        print "\nStep 6: Close MQTT session"
        SagSendAT(uart_com, 'AT+KMQTTCLOSE=%s\r' % session)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 5000)

    print "\nStep 7: Delete the MQTT connection"
    for session in range (1, max_session):
