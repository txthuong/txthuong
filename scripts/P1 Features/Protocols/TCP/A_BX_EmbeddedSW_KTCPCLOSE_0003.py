...
# A_BX_EmbeddedSW_KTCPCLOSE_0003                To check URC "+KTCP_NOTIF: <session_id>,4" could be received in server module when client has been closed, and the connection is automatically disconnected
#
# Requirement
#   2 Euler modules
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KTCPCLOSE_0003
# -----------------------------------------------------------------------------------
    
test_ID = "A_BX_EmbeddedSW_KTCPCLOSE_0003"
...
try:
    ...
    print "***************************************************************************************************************"
    print "%s: To check URC +KTCP_NOTIF: <session_id>,4 could be received in server module when client has been closed, and the connection is automatically disconnected" % test_ID
    print "***************************************************************************************************************"
    ...
    SagSendAT(aux1_com, 'AT+KTCPSND=1,"A_BX_EmbeddedSW_KTCPCLOSE_0003"\r')
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    
    print '\nStep 13: AUX: Check URC "+KTCP_DATA" is received\n'
    SagWaitnMatchResp(uart_com, ['\r\n+KTCP_DATA: 2,30,A_BX_EmbeddedSW_KTCPCLOSE_0003\r\n'], 2000)
    
    print '\nStep 14: AUX: Close created session\n'
    SagSendAT(aux1_com, 'AT+KTCPCLOSE=1,1\r')
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)

    print '\nDUT: +KTCP_NOTIF: <session_id>,4 could be displayed\n'
    SagWaitnMatchResp(uart_com, ['\r\n+KTCP_NOTIF: 2,4\r\n'], 2000)
    
    print '\nStep 15: AUX: Checking TCP Connection Configuration after it is closed\n'
    SagSendAT(aux1_com, 'AT+KTCPCFG?\r')
    SagWaitnMatchResp(aux1_com, ['\r\n+KTCPCFG: 1,0,,0,"%s",1234,,1,0\r\n' %uart_tcp_ip], 2000)
    SagWaitnMatchResp(aux1_com, ['OK\r\n'], 2000)
    
    print '\nStep 16: DUT: Checking TCP Connection Configuration after client is disconnected\n'
    SagSendAT(uart_com, 'AT+KTCPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KTCPCFG: 1,1,,1,"",1234,,1,0\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['+KTCPCFG: 2,0,,2,1,"%s",*,1234,1,0\r\n' %aux1_tcp_ip], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
