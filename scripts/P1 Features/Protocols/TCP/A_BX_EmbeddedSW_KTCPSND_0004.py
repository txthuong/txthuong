...
# A_BX_EmbeddedSW_KTCPSND_0004                To check 10KB data can be sent from server module to client module
#
# Requirement
#   2 Euler modules
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KTCPSND_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KTCPSND_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: To check 10KB data can be sent from server module to client module' % test_ID
    print "***************************************************************************************************************"
    ...
    SagSendAT(aux1_com, 'AT+KTCPCFG=,1,,%s\r' % tcp_port)
    SagWaitnMatchResp(aux1_com, ['\r\n+KTCPCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    ...
    SagSendAT(uart_com, 'AT+KTCPCFG=,0,"%s",%s\r' % (aux1_ip_address, tcp_port))
    SagWaitnMatchResp(uart_com, ['\r\n+KTCPCFG: 1\r\n'], 3000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 3000)
    ...
    while sent_data_length < 10000:
        SagSendAT(aux1_com, 'AT+KTCPSND=2,"%s"\r' % data)
        SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
        if SagWaitnMatchResp(uart_com, ['\r\n+KTCP_DATA: 1,%s,%s\r\n' % (len(data), data)], 10000):
            print '-----> %s bytes data were received successfully' % sent_data_length
        else:
            print '-----> Problem: Fail to receive data from TCP server !!!'
        sent_data_length += len(data)

    print '\nStep 7: Close TCP session'
    print 'On DUT...'
