...
# A_BX_EmbeddedSW_KTCPSND_0009                To check 100KB data can be send out successfully by "+KTCPSND"
#
# Requirement
#   1 Euler modules
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KTCPSND_0009
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_KTCPSND_0009"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: To check 100KB data can be send out successfully by "+KTCPSND"' % test_ID
    print "***************************************************************************************************************"
    ...
    SagWaitnMatchResp(uart_com, ['+KTCPCFG: 1,1,,2,1,"%s",*,%s,1,0\r\n' % (tcp_server, tcp_port)], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)

    data = '1234567890abcdefghijklmno'
    sent_data_length = len(data)

    print '\nStep 4: Send 100Kb data to DUT CLIENT'
    while sent_data_length < 100000:
        SagSendAT(uart_com, 'AT+KTCPSND=1,"%s"\r' % data)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
        if SagWaitnMatchResp(uart_com, ['\r\n+KTCP_DATA: 1,%s,%s\r\n' % (len(data), data)], 10000):
            print '-----> %s bytes data were sent successfully' % sent_data_length
        else:
            print '-----> Problem: Fail to sent data to TCP server !!!'
        sent_data_length += len(data)
