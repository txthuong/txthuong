...
# A_BX_EmbeddedSW_HTTPSPOST_0020           Check that +KHTTPSPOST works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256
#
# Requirement
#   1 Euler module
#   1 AP running at 2.4GHz band
#   1 Python HTTPS server supports TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256
#
# Author: txthuong
#
# Jira ticket:
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPSPOST_0020
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPSPOST_0020"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSPOST works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256' % test_ID
    print "***************************************************************************************************************"
    ...
    SagSleep(5000)
    # Start HTTPS service
    https_cmd = 'cmd /c start httpd -f conf/httpd-TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256.conf'
    ...
    supported_cipher_suite = (0, 14)

    print "\nCheck that +KHTTPSPOST works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256\n"
    for cipher_suite in range(0, 79):
        ...
        data = 'This line is HTTPSPOST data test..........'
        ...
        print "\nStep 6: Running +KHTTPSPOST with <cipher_suite> %d..." % cipher_suite
        connected = False
        ...
        if cipher_suite in supported_cipher_suite:
            ...
            if not connected:
                print 'Problem: +KHTTPSPOST does not handle server with supported <cipher_suite> preperly !!!\n'
                SagSendAT(uart_com, '+++')
                SagSleep(1500)
                if not SagWaitnMatchResp(uart_com, ["*\r\nOK\r\n", "*\r\nERROR\r\n", "*\r\n+CME ERROR: *\r\n"], 5000, update_result="not_critical"):
                    SagSendAT(uart_com, 'AT\r')
            ...
            if not match_result:
                if SagMatchResp(resp, ["\r\nCONNECT\r\n"]):
                    print 'Problem: +KHTTPSPOST works with server by unsupported <cipher_suite> !!!\n'
                    SagSendAT(uart_com, '+++')
                    SagSleep(3000)
                    if not SagWaitnMatchResp(uart_com, ["*\r\nOK\r\n", "*\r\nERROR\r\n", "*\r\n+CME ERROR: *\r\n"], 5000):
                        print "\nPROBLEM: +KHTTPSPOST Write command does not process +++ properly.\n"
                else:
                    print '\nPROBLEM: +KHTTPSPOST does not hanlde server by unsupported <cipher_suite> properly !!!\n'
