...
# A_BX_EmbeddedSW_HTTPSHEAD_0027         Check that +KHTTPSHEAD works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPSHEAD_0027
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPSHEAD_0027"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSHEAD works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nCheck that +KHTTPSHEAD works with a server by <cipher_suite> TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256"
    for cipher_suite in range(0, 79):
        ...
        if cipher_suite in supported_cipher_suite:
            ...
            if not SagWaitnMatchResp(uart_com, ['+KHTTPSCFG: 1,,"%s",%s,0,%d,1,,,0,0,2,2\r\n' % (https_server2, https_port, cipher_suite)], 2000):
                print 'Problem: +KHTTPSHEAD works with unsupported <cipher_suite> HTTPS configuration !!!\n'
                SagSendAT(uart_com, "AT+KHTTPSCLOSE=1\r")
            SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
