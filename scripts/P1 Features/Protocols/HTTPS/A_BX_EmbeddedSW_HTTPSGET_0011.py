...
# A_BX_EmbeddedSW_HTTPSGET_0011        Check that AT+KHTTPSGET command can access a HTTPS server by plain IP address
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPSGET_0011
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPSGET_0011"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that AT+KHTTPSGET command can access a HTTPS server by plain IP address' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 2: Configure a HTTPS connection by plain IP address"
    SagSendAT(uart_com, 'AT+KHTTPSCFG=,%s\r' % https_server_ip_address)
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query HTTPS configuration"
    SagSendAT(uart_com, 'AT+KHTTPSCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",443,0,0,1,,,0,0\r\n' % https_server_ip_address], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 4: Perform +KHTTPSGET to HTTPS server by plain IP address"
