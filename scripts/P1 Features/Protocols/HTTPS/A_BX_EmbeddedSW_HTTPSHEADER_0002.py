...
# A_BX_EmbeddedSW_HTTPSHEADER_0002         Check that +KHTTPSHEADER can add fields to HTTPS Request Header
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPSHEADER_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPSHEADER_0002"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSHEADER can add fields to HTTPS Request Header' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 5: Modify HTTPS header"
    connected = False
    SagSendAT(uart_com, 'AT+KHTTPSHEADER=1\r')
    if SagWaitnMatchResp(uart_com, ["\r\nCONNECT\r\n"], 3000):
        connected = True
        SagSendAT(uart_com, 'From: euler@example.net\r\n')
    ...
    print "\nStep 6: Run +KHTTPSGET to check HTTPS header fields"
    SagSendAT(uart_com, 'AT+KHTTPSGET=1,"/headers"\r')
    SagWaitnMatchResp(uart_com, ['\r\nCONNECT\r\n'], 5000)
    SagWaitnMatchResp(uart_com, ['HTTP/1.1 200 OK\r\n'], 5000)
    SagWaitnMatchResp(uart_com, ['*"headers": {*"From": "euler@example.net",*"Host": "%s"*}' % https_server], 2000)
    SagWaitnMatchResp(uart_com, ['*\r\nOK\r\n'], 2000)
