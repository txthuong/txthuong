...
# A_BX_EmbeddedSW_HTTPSGET_0013          Check that +KHTTPSGET works with a server by <cipher_suite> RC4-SHA
#
# Requirement
#   1 Euler module
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSGET works with a server by <cipher_suite> RC4-SHA' % test_ID
    print "***************************************************************************************************************"
    ...
    SagSleep(5000)
    https_cmd = 'cmd /c start python httpsd.py -x ./server.pem -c RC4-SHA -p '+str(https_port)
    ...
    print "\nCheck that +KHTTPSGET works with supported <cipher_suite> RC4-SHA"
    for cipher_suite in all_cipher_suite:
        ...
        if cipher_suite in supported_cipher_suite:
            ...
            if not SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",%s,1,%d,1,,,0,0\r\n' % (https_server2, https_port, cipher_suite)], 2000):
                print 'Problem: +KHTTPSGET works with unsupported <cipher_suite> HTTPS configuration !!!\n'
                SagSendAT(uart_com, "AT+KHTTPSCLOSE=1\r")
            SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
