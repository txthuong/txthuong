...
# A_BX_EmbeddedSW_HTTPSHEAD_0012         Check that +KHTTPSHEAD works with a server by <cipher_suite> DES_CBC_SHA
#
# Requirement
#   1 Euler module
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSHEAD works with a server by <cipher_suite> DES_CBC_SHA' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nCheck that +KHTTPSHEAD works with supported <cipher_suite> DES_CBC_SHA"
    for cipher_suite in all_cipher_suite:
        ...
        if cipher_suite in supported_cipher_suite:
            ...
            if not SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",%s,1,%d,1,,,0,0\r\n' % (https_server2, https_port, cipher_suite)], 2000):
                print 'Problem: +KHTTPSHEAD work with unsupported <cipher_suite> HTTPS configuration !!!\n'
                SagSendAT(uart_com, "AT+KHTTPSCLOSE=1\r")
            SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
