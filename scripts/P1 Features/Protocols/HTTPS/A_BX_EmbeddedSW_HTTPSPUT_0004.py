...
# A_BX_EmbeddedSW_HTTPSPUT_0004            Check that +KHTTPSPUT properly handles invalid domain name
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPSPUT_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPSPUT_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSPUT properly handles invalid domain name' % test_ID
    print "***************************************************************************************************************"
    ...
    invalid_donmain = 'www.example.example'

    print "\nStep 2: Configure a HTTPS connection with an invalid domain name"
    SagSendAT(uart_com, 'AT+KHTTPSCFG=,%s\r' % invalid_donmain)
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query HTTPS configuration"
    SagSendAT(uart_com, 'AT+KHTTPSCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",443,0,0,1,,,0,0\r\n' % invalid_donmain], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 4: Run +KHTTPSPUT to send data to HTTPS server"
