...
# A_BX_EmbeddedSW_HTTPCNX_0007           Check that +KHTTPCNX supports a non-80 HTTP port
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPCNX_0007
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPCNX_0007"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPCNX supports a non-80 HTTP port' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 2: Configure a HTTP connection with non-80 HTTP port"
    SagSendAT(uart_com, 'AT+KHTTPCFG=,%s,%s\r' % (http_server2, http_port))
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query HTTP configuration"
    SagSendAT(uart_com, 'AT+KHTTPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPCFG: 1,,"%s",%s,0,,,0,0\r\n' % (http_server2, http_port)], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
