...
# A_BX_EmbeddedSW_SRBCSMARTSTART_0010      Check if module can send massive data through BLE connection by +SRBCSMARTSTART command
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTSTART_0010
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTSTART_0010"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check if module can send massive data through BLE connection by +SRBCSMARTSTART command" % test_ID
    print "***********************************************************************************************************************"
    ...
    mtu = int (min (aux1_max_mtu, dut_max_mtu))
    ...
    print '\nStep 4: Send data with various length to AUX1'
    ...
    if not SagWaitnMatchResp(uart_com, ['\r\nCONNECT\r\n'], 2000):
        ...
        for count in range (1, 1000):
            ...
            remainder = len(data) % buff
            quotient = len(data)//buff
            print '\nData length: %s ...' % count
            for i in range (0, quotient):
                SagSendAT(uart_com, "%s" % data[buff*i:buff*(i+1)])
            if remainder != 0:
                SagSendAT(uart_com, "%s" % data[buff*quotient:len(data)+1])
            remainder = len(data) % mtu
            quotient = len(data)//mtu
            for i in range (0, quotient):
                SagWaitnMatchResp(aux1_com, ['+SRBCSMARTRECV: 1,1,"%s"\r\n' % data[mtu*i:mtu*(i+1)]], 2000)
            if remainder != 0:
                SagWaitnMatchResp(aux1_com, ['+SRBCSMARTRECV: 1,1,"%s"\r\n' % data[mtu*quotient:len(data)+1]], 2000)
