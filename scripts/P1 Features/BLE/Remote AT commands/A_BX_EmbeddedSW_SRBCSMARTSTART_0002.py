...
# A_BX_EmbeddedSW_SRBCSMARTSTART_0002      Verify command +SRBCSMARTSTART has <session_id>  support range 1-64
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTSTART_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTSTART_0002"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Verify command +SRBCSMARTSTART has <session_id>  support range 1-64" % test_ID
    print "***********************************************************************************************************************"
    ...
    for session in range (1, 65):
        ...
        data = 'Euler Testing'
        print '\nStep 5: Execute +SRBCSMARTSTART to send data to AUX with session id %s' % session
        ...
        print '\nStep 8: Configure a BLE configuration with dummy BT Mac Address'
        if session < 10:
        ...
        SagWaitnMatchResp(uart_com, ['\r\n+SRBLECFG: %s,0,"%s",%s' % (session, bt_mac_address, dut_max_mtu)], 2000)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

        if VarGlobal.statOfItem == "NOK":
            print '----> Failed to test SRBCSMARTSTART with session id %s' % session
            break

    print '\nStep 9: Query BLE configure'
    SagSendAT(uart_com, "AT+SRBLECFG?\r")
    for session in range (1, 65):
        if session < 10:
            bt_mac_address = 'aa:bb:cc:33:22:0%s' % session
    ...
    print '\nStep 10: Configure one more BLE connections'
    SagSendAT(uart_com, 'AT+SRBLECFG="aa:bb:cc:33:22:65"\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 912\r\n'], 2000)

    print '\nStep 11: Delete 64th session'
    for session in range (1, 65):
        SagSendAT(uart_com, "AT+SRBLEDEL=%s\r" % session)
        SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print '\nStep 12: Query BLE configuration'
    print 'On DUT...'
    SagSendAT(uart_com, "AT+SRBLECFG?\r")
    if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000):
