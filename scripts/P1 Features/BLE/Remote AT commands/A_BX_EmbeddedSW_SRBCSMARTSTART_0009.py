...
# A_BX_EmbeddedSW_SRBCSMARTSTART_0009      Check +SRBCSMARTSTART with local device does not support BC Smart Data
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTSTART_0009
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTSTART_0009"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check +SRBCSMARTSTART with local device does not support BC Smart Data" % test_ID
    print "***********************************************************************************************************************"
    ...
    print '\nOn DUT...'
    print 'Step 2: Disable BC Smart profile support'
    SagSendAT(uart_com, 'AT+SRBLE="%s",%s,0\r' % (dut_bt_name, dut_max_mtu))
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print '\nStep 3: Reset to apply BLE configure'
    SagSendAT(uart_com, 'AT+RST\r')
    SagWaitnMatchResp(uart_com, ['*\r\nREADY\r\n'], 2000)
    SagSendAT(uart_com, 'AT\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print '\nStep 4: Query BLE configure'
    SagSendAT(uart_com, "AT+SRBLE?\r")
    SagWaitnMatchResp(uart_com, ['\r\n+SRBLE: "%s",%s,0,0\r\n' % (dut_bt_name, dut_max_mtu)], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)

    print '\nStep 5: Configure an BLE connection to AUX1'
    ...
    if not SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000):
        print '----> Problem: Module did not return ERROR when sending data without BC Smart profile support !!!'
        SagSendAT(uart_com, '+++')
        if not SagWaitnMatchResp(uart_com, ['\r\nOK\r\n', '\r\nERROR\r\n', '\r\n+CME ERROR: *\r\n'], 2000):
...
SagSendAT(uart_com, 'AT+SRBLE="%s",23,1\r' % dut_bt_name)
SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
SagSendAT(uart_com, 'AT+RST\r')
SagWaitnMatchResp(uart_com, ['*\r\nREADY\r\n'], 2000)

print '\nSetting AUX1 configure to default'
SagSendAT(aux1_com, 'AT+SRBLE="%s",23,1\r' % aux1_bt_name)
SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
