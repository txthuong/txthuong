...
# A_BX_EmbeddedSW_SRBCSMARTCMD_0005        Check if module can send command multiple times
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTCMD_0005
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTCMD_0005"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check if module can send command multiple times" % test_ID
    print "***********************************************************************************************************************"
    ...
    command = 'AT+SRWAPCFG= "BX31-20034","eulerxyz",3,1,0,100'
    expect_response = ['\r\nOK\r\n']

    print '\nStep 5: Send AT command to remote BC Smart Server in multiple times'
    for i in range(0, len(command)):
        SagSendAT(uart_com, 'AT+SRBCSMARTCMD=1,"%s"\r' % command[i].replace('"', '\\22'))
        SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
    SagSendAT(uart_com, 'AT+SRBCSMARTCMD=1,"\\0d"\r')
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
