...
# A_BX_EmbeddedSW_SRBCSMARTCMD_0004        Check if module can send command has that size bigger than MTU-3
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTCMD_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTCMD_0004"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check if module can send command has that size bigger than MTU-3" % test_ID
    print "***********************************************************************************************************************"
    ...
    print '\nStep 5: Execute AT command on remote BC Smart Server'
    command = ''
    while len(command) < mtu:
        command = 'A' + command
        expect_response = ['\r\nERROR\r\n']
        print 'Execute command with size %s' % (len(command) + 1)
        SagSendAT(uart_com, 'AT+SRBCSMARTCMD=1,"%s\\0d"\r' % command)
        if len(command) < mtu-3:
            SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
            SagWaitnMatchResp(aux1_com, ['+SRREMCMD: "%s"\r' % command], 2000)
            SagWaitnMatchResp(aux1_com, expect_response, 5000)
