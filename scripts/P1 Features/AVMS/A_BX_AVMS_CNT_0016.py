...
# A_BX_AVMS_CNT_0016           The user shall be able to register the device to the AVMS server. The user shall be able to de-register the device from the AVMS session
#
# Requirement
#   1 Euler module
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: The user shall be able to register the device to the AVMS server. The user shall be able to de-register the device from the AVMS session" % test_ID
    print "***********************************************************************************************************************"
    ...
    print "\nStep 5: Query status of connection to the Device Services server"
    SagSendAT(uart_com, 'AT+WDSS?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+WDSS: 1,1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 6: Release the connection to the Device Services server"
    SagSendAT(uart_com, 'AT+WDSS=1,0\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 4000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 8\r\n'], 10000)

    print "\nStep 7: Query status of connection to the Device Services server"
    SagSendAT(uart_com, 'AT+WDSS?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+WDSS: 1,0\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
