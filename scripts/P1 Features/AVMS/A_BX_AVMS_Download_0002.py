...
# A_BX_AVMS_Download_0002         Verify that firmware upgrading will be resumed if the hardware resets while downloading is in progress
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_AVMS_Download_0002
# -----------------------------------------------------------------------------------

test_ID = "A_BX_AVMS_Download_0002"
...
try:
    ...
    for iPercent in range (0, 101):
        SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 18,%d\r\n' % iPercent], 7000)
        if iPercent == 30:
            print "\nStep 8: Power Off (Unplug USB cable) module while connection is still opened"
            ...
            if SagWaitnMatchResp(uart_com, ['\r\n+SRWSTASTATUS: 1,"%s","%s",*,*\r\n' % (wifi_ssid, wifi_mac_addr)], 20000):
                ...
                raise Exception("---->Problem: Module cannot reconnect to WI-FI AP after reboot!!!")
