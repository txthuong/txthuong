...
# A_BX_AVMS_Download_0004         Verify that the firmware upgrade is continued if user releases session while downloading is in progress
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_AVMS_Download_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_AVMS_Download_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Verify that the firmware upgrade is continued if user releases session while downloading is in progress' % test_ID
    print "***************************************************************************************************************"
    ...
    for iPercent in range (0, 101):
        if not SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 18,%d\r\n' % iPercent], 7000):
            break
        if iPercent == 30:
            print "\nStep 8: Release the session during upgrade process"
            SagSendAT(uart_com, 'AT+WDSS=1,0\r')
#            SagWaitnMatchResp(uart_com, ['*\r\nOK\r\n'], 10000)
#            SagWaitnMatchResp(uart_com, ['*\r\n+WDSI: 8\r\n'], 10000)
            ...
            percent = resp.split('\r\n')[1].split(',')[1]
            break
    for iPercent in range (int(percent)+1, 101):
        if not SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 18,%d\r\n' % iPercent], 7000):
            break
