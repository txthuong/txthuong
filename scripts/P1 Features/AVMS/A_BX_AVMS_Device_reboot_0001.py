...
# A_BX_AVMS_Device_reboot_0001    Verify that the device reboots after triggering the reboot System job on AVMS server
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_AVMS_Device_reboot_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_AVMS_Device_reboot_0001"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Verify that the device reboots after triggering the reboot System job on AVMS server" % test_ID
    print "***********************************************************************************************************************"
    ...
    print "\nStep 6: Create reboot system job on AVMS server"
    uid_operation = my_system.reboot()

    print "\nStep 7: Initiate a connection to the Device Services server to trigger the job"
    SagSendAT(uart_com, 'AT+WDSS=1,1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 4\r\n'], 20000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 6\r\n'], 20000)
    ...
    print "\nStep 8: Check the device reboots"
    SagWaitnMatchResp(uart_com, ['*\r\nREADY\r\n'], 20000)
    if SagWaitnMatchResp(uart_com, ['*\r\n+SRWSTASTATUS: 1,"%s","%s",*,*\r\n' % (wifi_ssid, wifi_mac_addr)], 20000):
        SagWaitnMatchResp(uart_com, ['\r\n+SRWSTAIP: "%s.*","%s","%s"\r\n' % (return_subnet(wifi_dhcp_gateway), wifi_dhcp_subnet_mask, wifi_dhcp_gateway)], 10000)
    else:
        print '---->Problem: Module cannot reconnect to Wi-Fi !!!'

    print "\nStep 9: Check operation status"
    if my_system.getOperationTasksDetails(uid_operation) != 'SUCCESS':
        print '----> Problem: Reboot system job is not SUCCESS !!!'
        VarGlobal.statOfItem = "NOK"
    else:
        print 'Reboot system job is SUCCESS'
