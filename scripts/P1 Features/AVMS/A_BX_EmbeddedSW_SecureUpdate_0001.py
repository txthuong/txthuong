...
# A_BX_EmbeddedSW_SecureUpdate_0001    Check that device could not update an unsigned software
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SecureUpdate_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SecureUpdate_0001"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that device could not update an unsigned software' % test_ID
    print "***************************************************************************************************************"
    ...
    print '\nStep 6: Create upgrade job on DM server with unsigned FW package'
    upgrade_job = mySystem.installApplication(package_unsigned_name ,package_unsigned_revision ,avms['model'])
    ...
    print "\nStep 8: Check Job status on AVMS server"
    time.sleep(10)
    job_status = mySystem.getOperationTasksDetails(upgrade_job)
    print '\n----> Job state is %s' % job_status
    if job_status in ["SUCCESS"]:
        print "\n-------->Problem: Upgrade Job success with unsigned FW package !!!"
    mySystem.cancelAllJobs()

    print "\nStep 9: Get revision information"
    SagSendAT(uart_com, 'ATI3\r')
    SagWaitnMatchResp(uart_com, ['*\r\nOK\r\n'], 2000)
    ...
    print "\n---->Info: Firmware version on server is: %s" % fw_rev
    print "\n---->Info: Firmware version in sample.cfg is: %s" % avms['fw_initial']
    if fw_rev == package_unsigned_revision:
        raise Exception("------>Problem: Upgrade FW job with unsigned package does not work properly !!!")
...
# Perform a reset to delete the downloaded package and restore setting
