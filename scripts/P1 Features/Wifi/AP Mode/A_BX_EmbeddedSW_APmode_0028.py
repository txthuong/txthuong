...
try:
    ...
    SagSendAT(uart_com, 'AT+SRWAPNETCFG=1,"172.16.0.254","%s.0","%s.0",720\r' %(return_subnet(wifi_dhcp_gateway), return_subnet(wifi_dhcp_gateway)))
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)
    
    print "\nStep 5: Enable DHCP with invalid values\n"
    SagSendAT(uart_com, 'AT+SRWAPNETCFG=1,"172.16.0.254","%s.0","%s.99",720\r' %(return_subnet(wifi_dhcp_gateway), return_subnet(wifi_dhcp_gateway)))
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)
