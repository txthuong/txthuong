...
# A_BX_EmbeddedSW_SRBCSMARTSEND_0004       Check syntax for command +SRBCSMARTSEND
#
# Requirement
#   2 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_SRBCSMARTSEND_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_SRBCSMARTSEND_0004"
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: Check syntax for command +SRBCSMARTSEND" % test_ID
    print "***********************************************************************************************************************"
    ...
    print '\nStep 4: Check +SRBCSMARTSEND test command'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=?\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print '\nStep 5: Check +SRBCSMARTSEND execute command'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print '\nStep 6: Check +SRBCSMARTSEND read command'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND?\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    data = 'Test BLE .....'

    print '\nStep 7: Check +SRBCSMARTSEND write command with out-range and invalid parameter <session id>'
    session_id = [-1, 0, 65, 'a', 'A', '*', '#']
    for session in session_id:
        SagSendAT(uart_com, 'AT+SRBCSMARTSEND=%s,0,"%s"\r' % (session, data))
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)

    print '\nStep 8: Check +SRBCSMARTSEND write command with not configured <session id>'
    session_id = [2, 3, 10, 20, 64]
    for session in session_id:
        SagSendAT(uart_com, 'AT+SRBCSMARTSEND=%s,0,"%s"\r' % (session, data))
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 910\r\n'], 2000)

    print '\nStep 9: Check +SRBCSMARTSEND write command with invalid parameter <gatt role>'
    gatt_role = [-1, 2, 'a', 'A', '*', '#']
    for role in gatt_role:
        SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1,%s,"%s"\r' % (role, data))
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)

    print '\nStep 10: Check +SRBCSMARTSEND write command with invalid parameter <data>'
    ble_data = ['"', '"Hello', '"Hello""']
    for d in ble_data:
        SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1,0,"%s"\r' % d)
        SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print '\nStep 11: Check +SRBCSMARTSEND write command with missing all parameters'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print '\nStep 12: Check +SRBCSMARTSEND write command with missing parameter <data>'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1,0\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print '\nStep 13: Check +SRBCSMARTSEND write command with missing parameter <gatt role>, <data>'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print '\nStep 14: Check +SRBCSMARTSEND write command with extra parameter'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1,0,"%s",1\r' % data)
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 915\r\n'], 2000)

    print '\nStep 15: Check +SRBCSMARTSEND write command with valid parameters'
    SagSendAT(uart_com, 'AT+SRBCSMARTSEND=1,0,"%s"\r' % data)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    SagWaitnMatchResp(aux1_com, ['+SRBCSMARTRECV: 1,1,"%s"\r\n' % data], 2000)

    print '\nStep 16: Close BLE connection'
    print 'On DUT...'
    ...
    print '\nStep 17: Delete BLE configuration'
    print 'On DUT...'
    ...
    print '\nStep 18: Query BLE configuration'
    print 'On DUT...'
