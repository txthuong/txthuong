...
# A_BX_AVMS_Download_0003         Verify that the firmware upgrade is resumed if network loses while downloading is in progress
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_AVMS_Download_0003
# -----------------------------------------------------------------------------------

test_ID = "A_BX_AVMS_Download_0003"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Verify that the firmware upgrade is resumed if network loses while downloading is in progress' % test_ID
    print "***************************************************************************************************************"
    ...
    for iPercent in range (0, 101):
        if not SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 18,%d\r\n' % iPercent], 7000):
            break
        if iPercent == 30:
            print "\nStep 8: Disconnect to WI-FI during upgrade process"
            ...
            print "\nStep 10: Check that Upgrade FW job resume after default timer"
            if SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 9,*\r\n'], 900000):
                SagWaitnMatchResp(uart_com, ['*\r\n+WDSI: 18,%d\r\n' % iPercent], 20000)
            else:
                mySystem.cancelAllJobs()
                raise Exception("---->Problem: Upgrade FW job was not triggered after default retry timer !!!")
