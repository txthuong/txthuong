...
# A_BX_AVMS_CNT_0021           To test the device is able to connect to DM server if the Wi-Fi is disconnected during previous session
#
# Requirement
#   1 Euler module
...
try:
    ...
    print "***********************************************************************************************************************"
    print "%s: To test the device is able to connect to DM server if the Wi-Fi is disconnected during previous session" % test_ID
    print "***********************************************************************************************************************"
    ...
    print "\nStep 5: Disconnect module to AP while the session is still up"
    ...
    print "\nStep 6: Reconnect to WI-FI Network"
    SagSendAT(uart_com, 'AT+SRWSTACON=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    if SagWaitnMatchResp(uart_com, ['*\r\n+SRWSTASTATUS: 1,"%s","%s",*,*\r\n' % (wifi_ssid, wifi_mac_addr)], 20000):
        SagWaitnMatchResp(uart_com, ['\r\n+SRWSTAIP: "%s.*","%s","%s"\r\n' % (return_subnet(wifi_dhcp_gateway), wifi_dhcp_subnet_mask, wifi_dhcp_gateway)], 10000)
