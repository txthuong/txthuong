...
# A_BX_AVMS_Update_0001           Check that device automatic upgrade of a nominal firmware package
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_AVMS_Update_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_AVMS_Update_0001"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that device automatic upgrade of a nominal firmware package' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 7: Initiate a connection to the Device Services server to trigger the upgrade job"
    SagSendAT(uart_com, 'AT+WDSS=1,1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 4000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 4\r\n'], 10000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 6\r\n'], 10000)
    ...
    if SagWaitnMatchResp(uart_com, ['*\r\n+SRWSTASTATUS: 1,"%s","%s",*,*\r\n' % (wifi_ssid, wifi_mac_addr)], 20000):
        ...
        raise Exception("---->Problem: Module cannot reconnect to WIFI network !!!")
    SagWaitnMatchResp(uart_com, ['*\r\n+WDSI: 4\r\n'], 10000)
    SagWaitnMatchResp(uart_com, ['*\r\n+WDSI: 6\r\n'], 30000)
    SagWaitnMatchResp(uart_com, ['*\r\n+WDSI: 23,1\r\n'], 10000)
    SagWaitnMatchResp(uart_com, ['\r\n+WDSI: 8\r\n'], 90000)

    print '\nStep 8: Check the status of installed application on server'
