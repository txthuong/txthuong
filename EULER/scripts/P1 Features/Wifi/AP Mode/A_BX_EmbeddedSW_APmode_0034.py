...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s: Use command +SRWAPNETCFG to configure the start and end assigned IP address with Class B (Ex: 192.168.0.2 - 192.168.0.101). The IP address pool size should be 101" % test_ID
    print "*****************************************************************************************************************"
    ...
    SagSendAT(uart_com, 'AT+SRWAPNETCFG=1,"192.168.0.1","192.168.0.2","192.168.0.102",720\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 4: Query network configuration\n"
    SagSendAT(uart_com, 'AT+SRWAPNETCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+SRWAPNETCFG: 1,"192.168.0.1","192.168.0.2","192.168.0.102",720\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print "\nStep 5: Re-configure DHCP with invalid values  (more than 101 IP address in DHCP IP pool)\n"
    SagSendAT(uart_com, 'AT+SRWAPNETCFG=1,"192.168.0.1","192.168.0.2","192.168.0.103",720\r')
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)
