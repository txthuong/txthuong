...
# A_BX_EmbeddedSW_HTTPSCNX_0002          Check that +KHTTPSCNX works with supported <cipher_suite> RC4-MD5
#
# Requirement
#   1 Euler module
#   1 AP running at 2.4GHz band
#   1 Python HTTPS server supports RC4-MD5
#
# Author: txthuong
#
# Jira ticket:
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSCNX works with supported <cipher_suite> RC4_128_MD5' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nCheck that +KHTTPSCNX works with supported <cipher_suite> RC4_128_MD5"
    for cipher_suite in all_cipher_suite:
        ...
        if not SagMatchResp(resp, ['\r\nOK\r\n']):
            if SagMatchResp(resp, ['\r\n+CME ERROR: 911\r\n']):
                print 'PROBLEM: Module still connect to HTTPS server with unsupported cipher suite !!!\n'
                SagSendAT(uart_com, "AT+KHTTPSCLOSE=1\r")
                SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
                SagSendAT(uart_com, 'AT+KHTTPSDEL=1\r')
                SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
            else:
                raise Exception ('\nPROBLEM: +HTTPSDEL command does not process properly')
