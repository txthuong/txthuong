...
# A_BX_EmbeddedSW_HTTPSCNX_0012          Check that +KHTTPSCNX properly handles invalid IP address
#
# Requirement
#   1 Euler module
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that +KHTTPSCNX properly handles invalid IP address' % test_ID
    print "***************************************************************************************************************"

    invalid_https_server = '198.51.100.27'

    print "\nStep 1: Query HTTPS configuration"
    SagSendAT(uart_com, 'AT+KHTTPSCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 2: Configure a HTTPS connection with invalid IP Address server"
    SagSendAT(uart_com, 'AT+KHTTPSCFG=,%s\r' % invalid_https_server)
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query HTTPS configuration"
    SagSendAT(uart_com, 'AT+KHTTPSCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",443,0,0,1,,,0,0\r\n' % invalid_https_server], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    ...
    if not SagWaitnMatchResp(uart_com, ['\r\n+KHTTPSCFG: 1,,"%s",443,0,0,1,,,0,0\r\n' % invalid_https_server], 2000):
        print 'Problem: Module cannot handle invalid HTTPS server properly !!!\n'
        SagSendAT(uart_com, 'AT+KHTTPSCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 6: Delete the HTTPS connection"
