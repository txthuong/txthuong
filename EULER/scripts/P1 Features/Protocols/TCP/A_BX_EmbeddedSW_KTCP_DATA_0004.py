...
# A_BX_EmbeddedSW_KTCP_DATA_0004                To check URC "+KTCP_DATA"  with <data> that shall not be displayed if no data is received
#
# Requirement
#   2 Euler modules
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KTCP_DATA_0004
# -----------------------------------------------------------------------------------
    
test_ID = "A_BX_EmbeddedSW_KTCP_DATA_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print "%s:To check notification with URC +KTCP_DATA shall not be received if no data is sent" % test_ID
    print "***************************************************************************************************************"
    ...
    print '\nStep 12: DUT: Send data to client\n'
    SagSendAT(uart_com, 'AT+KTCPSND=1,""\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 903\r\n'], 2000)
    
    print '\nStep 13: DUT: Check URC "+KTCP_DATA" is received\n'
    response = SagWaitResp(aux1_com, ['*'], 60000)
    if 'KTCP_DATA' in response:
        raise Exception('\r\nFAIL\r\n')
        VarGlobal.statOfItem = "NOK"
    if 'KTCP_DATA' not in response:
        print '\nNo URC "+KTCP_DATA" is received in DUT server\n'
