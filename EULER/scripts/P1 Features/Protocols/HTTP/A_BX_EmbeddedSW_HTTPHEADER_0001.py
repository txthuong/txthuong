...
# A_BX_EmbeddedSW_HTTPHEADER_0001               Check syntax for AT+HTTPHEADER command
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPHEADER_0001
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPHEADER_0001"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check syntax for AT+HTTPHEADER command' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 3: Check +KHTTPHEADER test command"
    SagSendAT(uart_com, "AT+KHTTPHEADER=?\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 4: Check +KHTTPHEADER action command"
    SagSendAT(uart_com, "AT+KHTTPHEADER\r")
    SagWaitnMatchResp(uart_com, ['\r\nERROR\r\n'], 2000)

    print "\nStep 5: Check +KHTTPHEADER read command"
    SagSendAT(uart_com, "AT+KHTTPHEADER?\r")
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 6: Check +KHTTPHEADER write command with missing parameter"
    SagSendAT(uart_com, 'AT+KHTTPHEADER=\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 917\r\n'], 2000)

    print "\nStep 7: Check +KHTTPHEADER write command with extra parameter"
    SagSendAT(uart_com, 'AT+KHTTPHEADER=1,1\r')
    SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 915\r\n'], 2000)

    print "\nStep 8: Check +KHTTPHEADER write command with not configured and out-range session"
    session_id = [-1, 0, 2, 65535, 65536]
    for session in session_id:
        SagSendAT(uart_com, 'AT+KHTTPHEADER=%s\r' % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 910\r\n'], 2000)

    print "\nStep 9: Check +KHTTPHEADER write command with invalid parameter <session_id>"
    session_id = ['a', 'A', '$', '*']
    for session in session_id:
        SagSendAT(uart_com, 'AT+KHTTPHEADER=%s\r' % session)
        SagWaitnMatchResp(uart_com, ['\r\n+CME ERROR: 916\r\n'], 2000)

    print "\nStep 10: Check +KHTTPHEADER write command with valid parameters"
    header = 'This is header'
    connected = False
    SagSendAT(uart_com, 'AT+KHTTPHEADER=1\r')
    if SagWaitnMatchResp(uart_com, ["\r\nCONNECT\r\n"], 3000):
        connected = True
        SagSendAT(uart_com, '%s' % header)
    ...
    print "\nStep 11: Query configured header"
    SagSendAT(uart_com, 'AT+KHTTPHEADER?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPHEADER: 1,%d\r\n' % len(header)], 2000)
    SagWaitnMatchResp(uart_com, ['This is header\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 12: Delete HTTP configuration"
