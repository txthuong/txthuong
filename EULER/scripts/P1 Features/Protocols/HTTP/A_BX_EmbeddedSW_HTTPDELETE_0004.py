...
# A_BX_EmbeddedSW_HTTPDELETE_0004        Check that AT+KHTTPDELETE command can access a HTTP server by HTTP 1.0
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPDELETE_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPDELETE_0004"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that AT+KHTTPDELETE command can access a HTTP server by HTTP 1.0' % test_ID
    print "***************************************************************************************************************"
    ...
    print "\nStep 2: Configure a HTTP connection with HTTP version 1.0"
    SagSendAT(uart_com, 'AT+KHTTPCFG=,%s,80,1\r' % http_server)
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 3: Query HTTP configuration"
    SagSendAT(uart_com, 'AT+KHTTPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPCFG: 1,,"%s",80,1,,,0,0\r\n' % http_server], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 4: Perform +KHTTPDELETE to HTTP server version 1.0"
    SagSendAT(uart_com, 'AT+KHTTPDELETE=1,"/delete"\r')
    SagWaitnMatchResp(uart_com, ['\r\nCONNECT\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['HTTP/1.0 200 OK\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['*%s' % http_server], 2000)
    SagWaitnMatchResp(uart_com, ['*\r\nOK\r\n'], 2000)
