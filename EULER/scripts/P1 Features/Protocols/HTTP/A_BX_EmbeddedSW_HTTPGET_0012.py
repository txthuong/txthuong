...
# A_BX_EmbeddedSW_HTTPGET_0012      Check that HTTP client can get server data 10000 times
#
# Requirement
#   1 Euler module
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_HTTPGET_0012
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_HTTPGET_0012"
...
try:
    ...
    print "***************************************************************************************************************"
    print '%s: Check that HTTP client can get server data 10000 times' % test_ID
    print "***************************************************************************************************************"

    print "\nStep 1: Query HTTP connection configuration"
    ...
    print "\nStep 3: Query HTTP connection configuration"
    SagSendAT(uart_com, 'AT+KHTTPCFG?\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KHTTPCFG: 1,,"%s",80,0,,,0,0\r\n' % http_server], 2000)
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print "\nStep 4: Check that HTTP client can get server data 10000 times"
    for i in range(1,10001):
        SagSendAT(uart_com, 'AT+KHTTPGET=1,\"/get?param=%s\"\r' % i)
        SagWaitnMatchResp(uart_com, ['\r\nCONNECT\r\n'], 5000)
        SagWaitnMatchResp(uart_com, ['HTTP/1.1 200 OK\r\n'], 5000)
        SagWaitnMatchResp(uart_com, ['*%s/get?param=%s*\r\n' % (http_server, i)], 5000)
    ...
    print "\nStep 6: Check +KHTTPDEL closed session"
